...
class Orchestrator:
    ...
    async def initialize(self) -> None:
        try:
            for node_id in self.nodes.keys():
                await self.create_subscriber(f"node/{node_id}/data", self.handle_node_data)
            logger.info(f"Orchestrator {self.id} initialized successfully")
        except Exception as e:
            logger.error(f"Error initializing orchestrator {self.id}: {e}", exc_info=True)
            raise
        ...
        while self.update_queue:
            ...
            if node_data.node_id in self.callbacks:
                try:
                    ...
                    logger.error(f"Error in callback for node {node_data.node_id}: {e}", exc_info=True)
    ...
    def cleanup(self):
        try:
            ...
            logger.error(f"Error during cleanup for orchestrator {self.id}: {e}", exc_info=True)
    ...
    async def create_subscriber(self, topic: str, callback: Callable[[Sample], None]) -> None:
        try:
            ...
            logger.error(f"Error creating subscriber for topic {topic}: {e}", exc_info=True)
            raise
    ...
    async def publish_node_config(self, node_id: str, config: NodeConfig) -> None:
        ...
        try:
            ...
            logger.error(f"Error publishing config for node {node_id}: {e}", exc_info=True)
            raise
    ...
    async def register_callback(self, node_id: str, callback: Callable[[NodeData], None]) -> None:
    ...
    async def remove_node(self, node_id: str) -> None:
        if node_id in self.nodes:
            ...
            logger.warning(f"Attempted to remove non-existent node {node_id} from orchestrator {self.id}")

    async def send_event_to_node(self, node_id: str, event: str, payload: Dict[str, Any]) -> None:
    ...
    async def process_node_updates(self) -> None:
        ...
        for node_id, state in self.nodes.items():
            if current_time - state.last_update > 5:  # 5 seconds threshold for updates
                await self.request_node_update(node_id)
    ...
    def publish_with_retry(self, topic: str, data: Any) -> None:
        if topic not in self.publishers:
            self.create_publisher(topic)
        self.publishers[topic].put(data)
